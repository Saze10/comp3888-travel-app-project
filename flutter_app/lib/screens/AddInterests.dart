...
class AddInterests extends StatefulWidget {
  String currentDay;
  AddInterests({Key? key, this.firebase, required this.currentDay})
      : super(key: key);
  var firebase;
...
class _AddInterestsState extends State<AddInterests> {
  ...
  Stream<List<DayPlan>>? get streamDayPlanData {
    try {
      return firebase
          .collection('day_plan')
          .snapshots()
          .map(dayPlanListFromSnapshot);
  ...
  void _resetInterests() {
    ...
    dayPlan!.tripInterests.forEach((key, value) {
      interests[key] = dayPlan!.tripInterests[key];
    });
  }
    ...
    return ElevatedButton(
      onPressed: () async {
        if (name == "Submit") {
          ...
          //print("!！！！！！！！！！！here I am!!!!!!!!!");
          //print(_tempMap);
          await UserServices.updateTripInterests(_tempMap);
          interests.clear;
